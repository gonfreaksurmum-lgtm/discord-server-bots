...
} = require("../../services/moderationPlus");
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s
        ...
        .addUserOption((o) =>
          o.setName("user").setDescription("Optional target").setRequired(false)
        )
    )
    ...
    .addSubcommand((s) =>
      s
        ...
        .addStringOption((o) =>
          o.setName("duration").setDescription("Example 30m").setRequired(true)
        )
        .addStringOption((o) =>
          o.setName("reason").setDescription("Reason").setRequired(false)
        )
    ...
    .addSubcommand((s) =>
      s
        ...
        .addBooleanOption((o) =>
          o.setName("enabled").setDescription("True = lock").setRequired(true)
        )
        .addChannelOption((o) =>
          o
            .setName("channel")
  ...
  async execute(interaction) {
    ...
    if (sub === "warn") {
      ...
      return interaction.reply({
        embeds: [successEmbed(`Warned ${user.tag}. Warning ID: **${id}**`)],
      });
    }
          ...
          infoEmbed(
            ...
            rows.length
              ? rows
                  ...
                  .map(
                    (r) =>
                      `#${r.id} • <t:${Math.floor(r.created_at / 1000)}:R> • ${r.reason}`
                  )
                  .join("\n")
              : "No warnings."
          ),
    ...
    if (sub === "clearwarnings") {
      ...
      return interaction.reply({
        embeds: [successEmbed(`Cleared warnings for ${user.tag}.`)],
      });
    }
    ...
    if (sub === "timeout") {
      ...
      return interaction.reply({
        embeds: [successEmbed(`Timed out ${user.tag} for ${durationText}.`)],
      });
    }
    ...
    if (sub === "lockdown") {
      ...
      return interaction.reply({
        embeds: [successEmbed(`${enabled ? "Locked" : "Unlocked"} ${channel}.`)],
        ephemeral: true,
      });
    }
    ...
    if (sub === "verification") {
      ...
      return interaction.reply({
        embeds: [successEmbed(`Verification panel sent to ${channel}.`)],
        ephemeral: true,
      });
    }
    ...
    if (sub === "autorole") {
      ...
      return interaction.reply({
        embeds: [successEmbed(`Auto role set to ${role}.`)],
        ephemeral: true,
      });
    }
  },
